...
<div class="w-100 px-3 py-2">
	<table class="table table-bordered border-dark w-100" id="contractor_info_table" style="min-width:1720px;">
...
<script>
	...
	$(document).ready(function() {
		$('#contractor_info_table').dataTable( {
			"processing": false,
			"serverSide": true,
			"responsive":  {
				details: true
			...
			"ordering": false,
			"ajaxSource": "/smarty/templates/$site_db/$templates/sub_modal/project/func06/overview_ms/server_case_contacts.php?site_db=$site_db&case_id=$case_id&organization_id=$contractor_id&organization_type=contractor",
			"info": false,
			"language": {
						"sUrl": "$dataTable_de"
					},
		...
		/* Init the table */
		oTable = $('#contractor_info_table').dataTable();
...
var contractor_info_myDraw = function(){
	var oTable;
	oTable = $('#contractor_info_table').dataTable();
	oTable.fnDraw(false);
}
